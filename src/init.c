...
** Last update Fri Mar 10 16:32:33 2017 Lucas Debouté
*/

#include "philosophers.h"

/*int	init_chopsticks(t_table *table)
{
  int	i;

  i = -1;
  if ((table->chopsticks = malloc(sizeof(pthread_mutex_t) * table->philosophers)) == NULL)
    return (1);
...
int	init_simulator(t_table *table)
{
  if (init_philosopher(table) != 0)
    /* init_chopsticks(table) != 0 )*/
    return (1);
  if ((table->threads = malloc(sizeof(pthread_t) * table->philosophers)) == NULL)
    return (1);
  return (0);
}
